...
> - One underappreciated reason for the popularity of BUGS is that users did not
  have to think much about PDFs
...
- $F^{-1}\left(0.5 \mid \dots \right)$ is the median of $\theta$, while
  ...
  upper quartiles
- Inverse CDFs rarely have explicit forms and are not in the Stan language
...
where $T_k\left(2p - 1\right)$ is the $k$-th shifted Chebyshev polynomial of the 1st kind

* Truncated Chebyshev series are used to _approximate_ a Lipschitz-continuous function, but
  here we are using it to _define_ a quantile function in much the same way that an entire
  function can be defined by its Taylor series as $K \uparrow \infty$
...
* Let $\mathbf{p}$ be an ordered vector of $K + 1$ cumulative probabilities and let
  $$\underbrace{\mathbf{T}\left(\mathbf{p}\right)}_{\left(K + 1\right) \times \left(K + 1\right)} = 
  \begin{bmatrix} 
    1 & T_1\left(2p_0 - 1\right) & \dots & T_K\left(2p_0 - 1\right) \\
    1 & T_1\left(2p_1 - 1\right) & \dots & T_K\left(2p_1 - 1\right) \\
    \dots & \dots & \dots & \dots \\
    1 & T_1\left(2p_K - 1\right) & \dots & T_K\left(2p_K - 1\right)
  \end{bmatrix}$$
* If $\boldsymbol{\theta} =  \mathbf{T}\left(\mathbf{p}\right) \mathbf{c}$
  is also given, then [Powley (2013)](http://www.metalogdistributions.com/images/Powley_Dissertation_2013-augmented.pdf)
  proves that 
  $\mathbf{c} = \mathbf{T}\left(\mathbf{p}\right)^{-1}\boldsymbol{\theta}$
  is unique, provided $\mathbf{T}\left(\mathbf{p}\right)^{-1}$ exists and
  $0 \leq \frac{\partial}{\partial p} \theta\left(p; \mathbf{c}\right)$ for all $p \in \left(0,1\right)$
  
> - Thus, if a user inputs $\mathbf{p}$ and $\boldsymbol{\theta}$, 
  we can solve for $\mathbf{c}$ and then check that $\theta\left(p; \mathbf{c}\right)$ is non-decreasing.
  The user need not deal with $\mathbf{c}$ and $T_k\left(2p - 1\right)$ directly.
...
## Prior Coefficients for $\sigma$ and $\sigma_1$

```{r, fig.width=11, fig.height=3.7, small.mar = TRUE, warning = FALSE}
q <- qno_name1(quantiles = c(0, 0.26, 0.5, 0.9, Inf), p = c(0, 0.25, 0.5, 0.75, 1))
curve(q(p), from = 1e-16, to = 1, n = 1001, xname = "p", ylab = expression(sigma), log = "y")
c_sigma <- make_c(quantiles = c(0, 0.26, 0.5, 0.9, Inf), p = c(0, 0.25, 0.5, 0.75, 1))
c_sigma_1 <- c_sigma
```

## Prior Coefficients for $\delta$ and $\delta_1$

```{r, fig.width=11, fig.height=3.7, small.mar = TRUE}
...
c_delta <- make_c(quantiles = c(0, 1 / 3, 0.5, 2 / 3, 1), p = c(0, 0.25, 0.5, 0.75, 1))
c_delta_1 <- c_delta
```


## Prior Coefficients for $\nu$, $\omega$, $\lambda$, and $\log \gamma$

```{r}
q_nu <- qno_name1(quantiles = c(0, 0.6, 1.0, 1.4, Inf), p = c(0, 0.25, 0.5, 0.75, 1))
q_omega <- qno_name1(quantiles = c(0, 0.016, 0.03, 0.05, 0.15), 
                     p = c(0, 0.25, 0.5, 0.75, 1))
q_lambda <- qno_name1(quantiles = c(0, 0.01, 0.02, 0.03, 0.05), 
                      p = c(0, 0.25, 0.5, 0.75, 1))
q_log_gamma <- qno_name1(quantiles = c(-2, 1, 3, 5, 7, 10),
                         p = c(0, 0.25, 0.5, 0.75, 0.9, 1))

c_nu <- make_c(quantiles = c(0, 0.6, 1.0, 1.4, Inf), p = c(0, 0.25, 0.5, 0.75, 1))
c_omega <- make_c(quantiles = c(0, 0.016, 0.03, 0.05, 0.15), 
                  p = c(0, 0.25, 0.5, 0.75, 1))
c_lambda <- make_c(quantiles = c(0, 0.01, 0.02, 0.03, 0.05), 
                   p = c(0, 0.25, 0.5, 0.75, 1))
c_log_gamma <- make_c(quantiles = c(-2, 1, 3, 5, 7, 10),
                      p = c(0, 0.25, 0.5, 0.75, 0.9, 1))
```
...
// defines no_name1_{dlu}b_icdf(real p, vector c)
#include no_name1.stan
data {
  int<lower = 0> T; // if T == 0, this draws from priors
  vector[T] log_Y; vector[T] log_K; 
  vector[T] log_E; vector[T] log_L;
  vector[5] c_sigma; vector[5] c_sigma_1; 
  vector[5] c_delta; vector[5] c_delta_1; 
  vector[5] c_nu; vector[5] c_omega;
  vector[5] c_lambda; vector[6] c_log_gamma;
}
parameters {
  vector<lower = 0, upper = 1>[8] p;
}  // cumulative probability primitives
transformed parameters {
  real sigma   = no_name1_lb_icdf(p[1], c_sigma);
  real sigma_1 = no_name1_lb_icdf(p[2], c_sigma_1);
  real delta   = no_name1_db_icdf(p[3], c_delta);
  real delta_1 = no_name1_db_icdf(p[4], c_delta_1);
  real nu      = no_name1_lb_icdf(p[5], c_nu);
  real omega   = no_name1_db_icdf(p[6], c_omega);
  real lambda  = no_name1_db_icdf(p[7], c_lambda);
  real log_gamma = no_name1_db_icdf(p[8], c_log_gamma);
}
model {
...
data_list <- with(GermanIndustry, list(T = nrow(GermanIndustry), log_Y = log_Y, 
                                       ...
                                       c_sigma = c_sigma, c_sigma_1 = c_sigma_1,
                                       c_delta = c_delta, c_delta_1 = c_delta_1,
                                       c_nu = c_nu, c_omega = c_omega,
                                       c_lambda = c_lambda, c_log_gamma = c_log_gamma))
```
```{r}
MLEs <- optimizing(CES, data = data_list, as_vector = FALSE, refresh = 0, seed = 12345)
round(rbind(unlist(MLEs$par[-1]), p = MLEs$par$p), digits = 3)
```
...
post <- sampling(CES, data = data_list, seed = 12345,
                 control = list(adapt_delta = 0.92, max_treedepth = 12), refresh = 0)
...
```{r, fig.width=11, fig.height=5.25, warning = FALSE, small.mar = TRUE}
